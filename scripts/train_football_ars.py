...
def main():
    ...
    parser.add_argument('--ball-start', type=float, nargs=2, default=[0.5, 0.0],
                        help='Curriculum ball start. Near goal = easy.')
    parser.add_argument('--attacker-start', type=float, nargs=2,
                        default=[-0.8, 0.0])
    ...
    if args.resume and os.path.exists(latest):
        ...
        with open(log_path, 'w', newline='') as f:
            csv.writer(f).writerow(
                ['iter', 'mean_ret', 'best_ret', 'sigma_r'])

    env_kwargs = dict(opponent_mode='static',
    ...
    with futures.ProcessPoolExecutor(
        ...
        for it in range(start_iter, start_iter + args.iters):
            ...
            for i, d in enumerate(deltas):
                for sign in (+1.0, -1.0):
                    payloads.append(
                        (matrix, mean, var, keys, d, sign, args.noise,
            ...
            if best_ret > best:
                best = best_ret
                save_policy(best_path, matrix, mean, var, keys, minimum,
                            maximum)
            np.savez(latest, matrix=matrix, mean=mean, var=var,
                     keys=np.array(keys), minimum=minimum, maximum=maximum,
                     best=best, iter=it)
            with open(log_path, 'a', newline='') as f:
                csv.writer(f).writerow(
                    [it, f'{mean_ret:.3f}', f'{best_ret:.3f}',
                     f'{sigma_r:.3f}'])
            print(f'iter {it}: mean={mean_ret:.2f} best_ep={best_ret:.2f} '
