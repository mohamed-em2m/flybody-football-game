...
def main():
    ...
    parser.add_argument('--opponent', default='static',
                        choices=['static', 'scripted', 'self_play'])
    parser.add_argument('--fps', type=int, default=30)
    parser.add_argument('--render-every', type=int, default=4,
                        help='Render every N-th control step.')
    parser.add_argument('--width', type=int, default=640)
    parser.add_argument('--height', type=int, default=480)
    args = parser.parse_args()

    env = football_vs(opponent_mode=args.opponent,
    ...
    while not timestep.last():
        ...
        if step % args.render_every == 0:
            frames.append(
                env.physics.render(height=args.height,
                                   width=args.width,
                                   camera_id=camera_id))
        step += 1
